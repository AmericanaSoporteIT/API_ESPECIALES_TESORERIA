...
def consulta_traslados_por_dia():
    try:
        ...
        WHERE x.codigo_estado <> {1}
          AND x.fecha_operacion = '{0}'""".format(fecha, CODIGO_ESTADO_ANULADO))
...
def consulta_traslados_generados():
    try:
        ...
        SELECT  x.codigo_traslado, 
                ...
                DATE_FORMAT(x.fecha_ing,'%d/%m/%Y %H:%i:%s') AS fecha_ing,
                CASE
                    WHEN x.codigo_estado IN ({0})  THEN 1
                    ELSE 0
                ...
                CASE
                    WHEN x.codigo_estado NOT IN ({0})  THEN 1
                    ELSE 0
                ...
                CASE
                    WHEN x.codigo_estado IN ({1})  THEN 1
                    ELSE 0
        ...
        INNER JOIN inf_estado_traslado_especiales2 y
        ON x.codigo_estado = y.codigo_estado_traslado 
        WHERE x.codigo_estado IN ({0},{1})
          AND x.fecha_operacion >= CURDATE()""".format(CODIGO_ESTADO_GENERADO, CODIGO_ESTADO_POR_RECEPCIONAR))
...
def consulta_traslados_generados_contabilidad():
    try:
        ...
        INNER JOIN inf_estado_traslado_especiales2 y
        ON x.codigo_estado = y.codigo_estado_traslado
        WHERE x.codigo_estado IN ({0},{1})""".format(CODIGO_ESTADO_GENERADO, CODIGO_ESTADO_POR_RECEPCIONAR))
...
def generar_traslado_especiales2():
    ...
    try:
        ...
        numeroRegistros = cursor.execute("""
        SELECT count(*) AS registros
        FROM inf_pedido x
        LEFT JOIN inf_traslado_detalle_especiales2 y
        ON x.empresa = y.empresa AND x.serie = y.serie AND x.pedido = y.pedido
        WHERE x.qsys_vendedor = {1}
        AND x.fecha =  '{0}'
        AND x.credito = 0
        AND x.estado <> 9
        AND (y.empresa IS NULL AND y.serie IS NULL AND y.pedido IS NULL)""".format(fecha, 332)).fetchone()[0]

        if numeroRegistros > 0:
            insertEncabezado = cursor.execute("""
            ...
            AND x.credito = 0
            AND x.estado <> 9
            AND (y.empresa IS NULL AND y.serie IS NULL AND y.pedido IS NULL)""".format(codigoTraslado, usuario, 332, fecha))
...
def consulta_traslados_para_importacion():
    try:
        ...
        INNER JOIN inf_estado_traslado_especiales2 y
        ON x.codigo_estado = y.codigo_estado_traslado
        WHERE x.codigo_estado in ({0},{1},{2})""".format(CODIGO_ESTADO_POR_RECEPCIONAR, CODIGO_ESTADO_RECEPCIONADO, CODIGO_ESTADO_DEPURADO))
...
def modificacion_detalle_trasladado():
    ...
    try:
        ...
        insertHistorialModificaciones =  cursor.execute("""
        ...
        ON x.empresa = y.empresa AND x.serie = y.serie AND x.pedido = y.pedido
        WHERE x.codigo_traslado = {1}
          AND x.estado = 1
          AND ((x.monto - cast(y.monto as decimal(18,2))) <> 0 OR y.estado = 9 OR y.qsys_vendedor <> {2} OR y.credito <> 0)""".format(usuario,codigo,332))

        updateCambios =  cursor.execute("""
        ...
        INNER JOIN ( SELECT x.empresa, 
                            ...
                            CASE
                                WHEN (y.estado = 9 OR y.qsys_vendedor <> {1} OR y.credito <> 0) THEN 0
                                ELSE 1
                            END AS codigo_estado
			         FROM inf_traslado_detalle_especiales2 x
			         INNER JOIN inf_pedido y
			         ON x.empresa = y.empresa AND x.serie = y.serie AND x.pedido = y.pedido
			         WHERE x.codigo_traslado = {0}
                      AND x.estado = 1
        ...
        SET a.monto = b.monto_actualizado, a.estado = b.codigo_estado""".format(codigo,332))
...
def consulta_ventas_al_credito_por_fecha():
    try:
        ...
        WHERE credito = 1
          AND fecha = '{0}'
          AND estado <> {1}""".format(fecha,CODIGO_ESTADO_ANULADO))
